...
class nnUNetSegClsDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        If load_classification_labels is True, the dataset will also load classification labels.
        ...
        for c in case_identifiers:
            ...
            if load_classification_labels:
                # extract classification label from filename itself. quiz_0_XXX -> classification_label = 0
                classification_label = int(c.split('_')[1])
                self.dataset[c]['classification_label'] = int(classification_label)
    ...
    def load_case(self, key):
        ...
        if 'classification_label' in entry.keys():
            classification_label = entry['classification_label']

        assert classification_label is not None, f'Classification label {classification_label} is not present in the dataset'

        return data, seg, classification_label, entry['properties']